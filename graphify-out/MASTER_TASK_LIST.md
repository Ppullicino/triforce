...
- 2026-07-17 — STATE-001: Implemented RunRegistry persistence and restoration. Created automatic JSONL-based run event logging (using file permissions mode 0600) under `~/.local/share/triforce/runs` or custom runsDir, index persistence in `index.json`, serialized file operations per run using a promise chain, in-memory replay buffer caps, and crash recovery that transitions runs in `running` state to `failed` with synthetic state events. Wired load() on server boot in `server.js` and added robust integration tests in `test/run-registry.test.js`. Evidence: `npm run test:server` (44/44 passed). Deviations: none.
- 2026-07-17 — STATE-002: Implemented run cancellation end-to-end. Extended protocol schemas, bumped minor protocol version to 1.1.0 and added cancellation capabilities feature. Threaded AbortSignal through the shared pipeline, Agent.call() (killing CLI child processes via tree-killing), runSandboxed, and runWorkspaceTest (systemctl killing sandbox/workspace units). Added cancellation handler to RunRegistry, wired the ws cancel command, and wrote E2E and unit test coverage verifying process termination and cancelled run state updates. Evidence: `npm run test:server` (50/50 passed). Deviations: none.
- 2026-07-17 — STATE-003: Created unified `models.js` module acting as a single source of truth for model specifications, RATES, and ALLOWED_MODELS. Consolidated all token usage/cost calculations. Added startup validation checks for `models.config.json` that emit process warnings when configurations use models outside the catalog, and implemented a one-time console warning per unknown model in the cost calculation pipeline. Removed the module-level `latestUsage` global in `server.js`, moving the state tracking into `RunRegistry` (populating `run.usage` dynamically from runtime events and disk recoveries, queried via `getLatestUsage()`). Wrote new test coverage in `test/orchestrator.test.js`, `test/run-registry.test.js`, and `test/server-protocol.test.js`. Evidence: `npm run test:server` (53/53 passed). Deviations: none.
- 2026-07-17 — FS-001: Implemented Git isolation for Mode 3 workspace directories. Modified `createWorkspace` to accept and reuse `existingWorkspace` across iterations of the same run, cleaning the directory of non-git/non-node_modules files before writing the new iteration files. Added automatic `git init`, `git add -A`, and `git commit` (with `--allow-empty`) after each iteration. Added feature-detection/graceful degradation for environments without `git`. Added git diff generation of the last iteration changes, bounded to 20KB, and exposed it to the Reviewer's prompt if `iteration > 1`. Added a unit test in `test/workspace.test.js` validating workspace reuse, git initialization/commits, and diffing. Evidence: `npm run test:server` (54/54 passed). Deviations: none.
- 2026-07-17 — FS-003: Implemented workspace retention/GC. Added `gcWorkspaces(root, { keep, protect })` to `workspace.js`: keeps the N newest workspace dirs (default 20, `TRIFORCE_WORKSPACE_KEEP` override), matches only names generated by `createWorkspace` (ISO-timestamp + 10-hex pattern) sorted lexicographically, skips foreign names, refuses symlinks/paths resolving outside the root (realpath containment before rm), and never deletes protected dirs. Wired GC into `pipeline.js` after every Mode 3 run (success and failure paths, protecting the current run's workspace via `activeWorkspaceDir`) — covering both server and CLI transports — and into `server.js` startup. Added 4 tests in `test/workspace.test.js`: 25 dirs → newest 20 kept + foreign dir skipped; protected oldest dir survives; env-var keep + missing root tolerated; symlink to outside dir never followed (target intact). Evidence: `node --test test/workspace.test.js` (17/17 passed); `npm run test:server` (59/59 passed). Deviations: none.
- 2026-07-17 — OPS-001: Implemented graceful shutdown in `server.js`. SIGTERM/SIGINT handler: sets `shuttingDown` (new WS `run` commands refused with `SERVER_SHUTTING_DOWN` protocol error — no schema change needed, `protocol_error` codes are free-form; new upgrade requests destroyed), cancels the active run via STATE-002 `RunRegistry.cancel()` and races its `completion` against `TRIFORCE_SHUTDOWN_GRACE_MS` (default 10s), kills lingering `triforce-sandbox-<pid>-*`/`triforce-workspace-<pid>-*` systemd units via `systemctl --user kill`, awaits `runRegistry.flush()`, terminates WS clients + `closeIdleConnections()` before `httpServer.close()` (keep-alive pitfall), exits 0. Second signal during shutdown force-exits 1; an unref'd deadline timer (grace+5s) force-exits if any step wedges. Checked `triforce.service`: it had NO `TimeoutStopSec` (implicit 90s default); pinned `TimeoutStopSec=30` with a comment so grace < stop timeout is explicit. Made `runValidationPipeline` duration tunable via `TRIFORCE_E2E_FAKE_PIPELINE_DELAY_MS` (test-only) so the integration test can SIGTERM mid-run. Integration test in `test/server-protocol.test.js`: fake 60s run started over WS, SIGTERM → exit 0 in ~0.3s (< grace), index.json persists run as `cancelled`, JSONL contains `run_state: cancelled`, `systemctl --user list-units triforce-sandbox-*` clean. Evidence: `node --test test/server-protocol.test.js` (5/5 passed); `npm run test:server` (60/60 passed); post-suite `systemctl --user list-units` shows no triforce units. Deviations: none.
- 2026-07-17 — OPS-002: Captured real usage/cost for CLI providers. `agent.js`: claude CLI now invoked with `-p --output-format json`; new exported `parseClaudeJsonEnvelope()` extracts text from the envelope's `result` field and sums `input_tokens` + cache creation/read tokens (real billed input) with `output_tokens`. codex CLI now invoked with `--json`; new exported `parseCodexJsonl()` tolerantly scans the JSONL event stream for the agent message text and the latest usage event. `_collectChild` gained an optional `parseOutput` argument: on parse failure or absence (agy has no structured output) it falls back to raw trimmed stdout with `usage: {0, 0, usageUnknown: true}` — so plain-text mode never regresses (pitfall a), and the existing MAX_PROVIDER_OUTPUT_BYTES cap still applies during stream collection, i.e. before any JSON.parse (pitfall b). `pipeline.js`: both costRecords builders (success + failure paths) now carry `usageUnknown: true` through to cost events. Updated the two `_codexCLIArgs` arg-shape tests for `--json`; added 4 stub-CLI tests in `test/agent.test.js` (PATH-prefixed executable stubs: claude JSON envelope → {15, 34} no flag; claude plain text → zeros + flag; codex JSONL → {100, 7}; agy → zeros + flag) and 1 pipeline test in `test/orchestrator.test.js` asserting cost records flag only the unknown-usage role. Evidence: `node --test test/agent.test.js` (16/16 passed); `node --test test/orchestrator.test.js` (10/10 passed); `npm run test:server` (65/65 passed). Deviations: none.
- 2026-07-17 — FS-002: Sandbox capability detection with explicit degradation. Added startup probing for `systemd-run --user true` with a 2-second timeout and robust handling of synchronous spawn/resolution exceptions. Added `sandbox` capability parameter (`'systemd' | 'unavailable'`) to dynamic server capabilities object returned via `/api/capabilities` and WebSocket capabilities exchanges. Wired runtime checks to WebSocket `run` command handler to reject commands with a `SANDBOX_UNAVAILABLE` protocol error and message when the sandbox capability is unavailable. Added tests in `test/server-protocol.test.js` verifying the capabilities and run rejection behaviors. Evidence: `npm run test:server` (55/55 passed). Deviations: none.
